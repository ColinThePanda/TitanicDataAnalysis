...
def write_analysis(data : list[TitanicData], filepath : str):
    with open(filepath, 'w') as file:
        ...
        best_gender = list(gender_map.keys())[list(gender_map.values()).index(max(gender_map.values()))]
        
        class_map = {"First" : survival_rate(first_class), "Second" : survival_rate(second_class), "Third" : survival_rate(third_class)}
        best_class = list(class_map.keys())[list(class_map.values()).index(max(class_map.values()))]
        
        age_group_map = {"Child" : survival_rate(children), "Adult" : survival_rate(adults), "Senior" : survival_rate(seniors)}
        best_age_group = list(age_group_map.keys())[list(age_group_map.values()).index(max(age_group_map.values()))]
...
def plot_data(data : list[TitanicData]):
    ...
    valid_int_ages = [round(float(person.age)) for person in valid_property(data, "age")]
    ...
    ax2.hist(valid_int_ages, bins=50, edgecolor="black")
